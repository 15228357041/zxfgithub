...
public class Paginator {
	...
	public String getHtml() {
		...
		if (page > 1) {
			if (url.contains("?")) {
				pages = pages.concat("<constant class=\"prev\" href=\"" + url + "&" + param + "=" + (page - 1) + "\">上一页</constant>\n");
			} else {
				pages = pages.concat("<constant class=\"prev\" href=\"" + url + "?" + param + "=" + (page - 1) + "\">上一页</constant>\n");
			}
		} else {
			// 特定需求可隐藏
			pages = pages.concat("<constant class=\"prev\" href=\"javascript:;\" style=\"color:#ccc\">上一页</constant>\n");
		}
		// 如果总页数大于要显示的个数，则拼接显示
		if (pageCount > step) {
			// 显示分页码
			...
			if (listBegin >= 2) {
				if (url.contains("?")) {
					pages = pages.concat("<constant href=\"" + url + "&" + param + "=1\">1</constant> ... \n");
				} else {
					pages = pages.concat("<constant href=\"" + url + "?" + param + "=1\">1</constant> ... \n");
				}
			}
			...
			if (pageCount - page >= page - listBegin) {
				for (int i = listBegin; i < (listBegin + step); i++) {
					if (i != page) {
						if (url.contains("?")) {
							pages = pages.concat("<constant href=\"" + url + "&" + param + "=" + i + "\">" + i + "</constant>\n");
						} else {
							pages = pages.concat("<constant href=\"" + url + "?" + param + "=" + i + "\">" + i + "</constant>\n");
						}
					} else {
				...
				if (listBegin + step <= pageCount) {
					if (url.contains("?")) {
						pages = pages.concat(" ... <constant href=\"" + url + "&" + param + "=" + pageCount + "\">" + pageCount + "</constant>\n");
					} else {
						pages = pages.concat(" ... <constant href=\"" + url + "?" + param + "=" + pageCount + "\">" + pageCount + "</constant>\n");
					}
				}
		...
		if (page < pageCount) {
			if (url.contains("?")) {
				pages = pages.concat("<constant class=\"next\" href=\"" + url + "&" + param + "=" + (page + 1) + "\">下一页</constant>\n");
			} else {
				pages = pages.concat("<constant class=\"next\" href=\"" + url + "?" + param + "=" + (page + 1) + "\">下一页</constant>\n");
			}
		} else {
			// 特定需求可隐藏
			pages = pages.concat("<constant class=\"next\" href=\"javascript:;\" style=\"color:#ccc\">下一页</constant>\n");
		}
		return pages;
	}
